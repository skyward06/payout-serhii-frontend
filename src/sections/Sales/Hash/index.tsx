        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (email) {
                router.push(
                  `${paths.auth.verifyResult}?${new URLSearchParams({ email: email!, paymentStatus: status === WaitTransactionStatus.Received ? 'success' : status === WaitTransactionStatus.Expired ? 'failed' : 'canceled' }).toString()}`
                );
              }
            }}
          >
