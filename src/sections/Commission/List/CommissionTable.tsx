...
export function CommissionTable({ status, customFilter }: Props) {
  ...
  useEffect(() => {
    fetchCommissions({
      variables: {
        filter: {
          ...graphQueryFilter,
          ...(status.toUpperCase() === CommissionType.Supernova
            ? { commissionType: CommissionType.Supernova, commission: { gt: 0 } }
