...
export function CalendarFiltersResult({ filters, totalResults, sx }: Props) {
  ...
  const handleRemoveDate = useCallback(() => {
    filters.setState({ start: null, end: null });
  }, [filters]);
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={filters.onResetState} sx={sx}>
      ...
      <FiltersBlock label="Date:" isShow={Boolean(filters.state.start && filters.state.end)}>
        <Chip
          {...chipProps}
          label={fDateRangeShortLabel(filters.state.start, filters.state.end)}
