...
import { customizeDate, formatUTCTime } from 'src/utils/format-time';
...
export function useCalendar() {
  ...
  const onResizeEvent = useCallback(
    (arg: EventResizeDoneArg, updateEvent: (eventData: any) => void) => {
      const { event } = arg;

      const isAllDay = event.startStr.split('T').length && event.endStr.split('T').length === 1;

      updateEvent({
        id: event.id,
        allDay: event.allDay,
        start: isAllDay
          ? customizeDate(event.startStr)
          : formatUTCTime(dayjs(event.startStr).utc().format('YYYY-MM-DDTHH:mm:ss')),
        end: isAllDay
          ? customizeDate(event.endStr)
          : formatUTCTime(dayjs(event.endStr).utc().format('YYYY-MM-DDTHH:mm:ss')),
      });
    },
    []
  );

  const onDropEvent = useCallback((arg: EventDropArg, updateEvent: (eventData: any) => void) => {
    const { event } = arg;
