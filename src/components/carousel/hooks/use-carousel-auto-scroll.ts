...
export function useCarouselAutoScroll(mainApi?: EmblaCarouselType): UseCarouselAutoPlayReturn {
  ...
  const onClickAutoplay = useCallback(
    (callback: () => void) => {
      ...
      const resetOrStop = (
        (autoScroll.options as any).stopOnInteraction === false ? autoScroll.reset : autoScroll.stop
      ) as () => void;
  ...
  const onTogglePlay = useCallback(() => {
    ...
    const playOrStop = (autoScroll as any).isPlaying()
  ...
  useEffect(() => {
    ...
    setIsPlaying((autoScroll as any).isPlaying());
    (mainApi as any)
